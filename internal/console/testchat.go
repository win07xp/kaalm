...
// ChatClient delivers one test-chat message through the gateway and returns
// the gateway's status code and body verbatim: the gateway's responses are
// exactly the documented wire contract, so the console relays rather than
// re-mapping.
type ChatClient interface {
	Chat(ctx context.Context, namespace, agent, userID, content string) (int, []byte, error)
}

// GatewayChatClient is the production ChatClient: POST /v1/test-chat on the
// gateway's cluster listener, presenting kaalm-console-tls over mTLS.
...
func (c *GatewayChatClient) Chat(ctx context.Context, namespace, agent, userID, content string) (int, []byte, error) {
	...
	url := strings.TrimSuffix(c.BaseURL, "/") + "/v1/test-chat"
