...
func TestChannelSecret_Branches(t *testing.T) {
	...
	bearer := &kaalmv1beta1.ChannelAuth{Type: authTypeBearer, SecretRef: &kaalmv1beta1.SecretKeyReference{Name: "bearer-secret", Key: "token"}}
	...
	if _, err := s.channelSecret(ctx, "team-a", &kaalmv1beta1.ChannelAuth{Type: authTypeBearer}); err == nil {
		t.Error("bearer without secretRef must error")
	}
	// HMAC with block resolves.
	hm := &kaalmv1beta1.ChannelAuth{Type: authTypeHMAC, HMAC: &kaalmv1beta1.ChannelHMAC{
	...
	if _, err := s.channelSecret(ctx, "team-a", &kaalmv1beta1.ChannelAuth{Type: authTypeHMAC}); err == nil {
		t.Error("hmac without block must error")
	}
	// Unknown type errors.
	if _, err := s.channelSecret(ctx, "team-a", &kaalmv1beta1.ChannelAuth{Type: "mystery"}); err == nil {
...
func TestVerifyHMACHeader(t *testing.T) {
	...
	// Undecodable hex.
	if verifyHMACHeader("zz-not-hex", &kaalmv1beta1.ChannelHMAC{}, expected) {
		t.Error("bad hex must fail")
	}
	// Undecodable base64.
	if verifyHMACHeader("!!!!", &kaalmv1beta1.ChannelHMAC{Encoding: "base64"}, expected) {
		t.Error("bad base64 must fail")
	}
	// Well-formed but wrong digest.
	if verifyHMACHeader(hex.EncodeToString([]byte("wrongwrongwrongwrong")), &kaalmv1beta1.ChannelHMAC{}, expected) {
