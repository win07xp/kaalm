...
func TestExtract(t *testing.T) {
	...
	// FromHeader.
	hdrName := "X-User"
	if got := extract(kaalmv1beta1.ChannelExtractor{FromHeader: &hdrName}, r, nil); got != "hdr-user" {
		t.Errorf("header extract = %q", got)
	}
	// FromHeader missing -> fallback.
	missing := "X-Absent"
	if got := extract(kaalmv1beta1.ChannelExtractor{FromHeader: &missing, Fallback: &fallback}, r, nil); got != "anon" {
		t.Errorf("fallback = %q", got)
	}
	// FromBody.
	path := "user.id"
	body := map[string]any{"user": map[string]any{"id": "b-user"}}
	if got := extract(kaalmv1beta1.ChannelExtractor{FromBody: &path}, r, body); got != "b-user" {
		t.Errorf("body extract = %q", got)
	}
	// No extractor and no fallback -> empty.
	if got := extract(kaalmv1beta1.ChannelExtractor{}, r, nil); got != "" {
