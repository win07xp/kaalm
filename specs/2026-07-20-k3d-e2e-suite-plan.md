...
In `main.go`, immediately after the server-start goroutine and before `<-ctx.Done()`, insert (the retry absorbs a startup race: completing at pod start can 401 the gateway's source-IP check before its Pod informer has indexed this pod):
