...
4. **Cert-file watch and reload** — inotify on Linux (fsnotify in Go, `aionotify`/`watchdog` in Python) on `$AGENTRY_TLS_CERT`, `$AGENTRY_TLS_KEY`, and `$AGENTRY_CA_CERT`. On `$AGENTRY_TLS_CERT` / `$AGENTRY_TLS_KEY` change, the template reloads both the server cert and the outbound client cert without process restart. On `$AGENTRY_CA_CERT` change, the template rebuilds the outbound HTTP client's `RootCAs` pool from the new bundle. Both reloads are required because cert-manager rotates leaf certs continuously, and trust-manager re-projects the CA ConfigMap during CA rotation; without watching the CA file, CA rotation eventually breaks outbound calls once all gateway leaves are signed by the new CA — the overlap window is finite.
