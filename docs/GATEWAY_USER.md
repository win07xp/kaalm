...
2a. **Payload size check**: the gateway rejects webhook payloads exceeding `maxMessageBodyBytes` (default: 1 MiB, configurable via Helm value `gateway.maxMessageBodyBytes`) with HTTP 413 before normalization. The check runs at the listener level on the raw request frame, **before path resolution and before the path-not-registered → `401` branch** — so `413` fires uniformly for any oversized POST to `:8080` regardless of whether the path is a registered AgentChannel. This prevents oversized payloads from consuming gateway resources or being forwarded to agent containers, and it preserves the path-existence threat model documented in [API_ENDPOINTS.md § Polling Fallback](./API_ENDPOINTS.md#polling-fallback) (the `413`-vs-`401` distinction must not leak which paths are hosted).
3. **Normalization**: the adapter translates the webhook payload into the Agentry message envelope. The `userId` is resolved using `AgentChannel.spec.webhook.userId` config (`fromHeader` or `fromBody`); if neither is configured or the value is absent, the configured `fallback` is used (empty string if omitted). The `content` field is resolved using `AgentChannel.spec.webhook.content` config (same shape as `userId`); when neither `fromHeader` nor `fromBody` is set, the gateway uses the raw inbound body, JSON-encoded as a string, as `content` — preserving the generic-webhook story for senders whose body shape Agentry has no a-priori knowledge of. If `fromBody` is configured for either field and the body cannot be parsed as JSON, the gateway rejects the request with `400 Bad Request` (`error.type: invalid_request`) before delivering to the agent. `attachments` and `metadata` are populated by adapter-specific code (empty `[]` and `{}` for the v1 generic webhook adapter; v1.1 Discord and WhatsApp adapters fill them per platform). See [AgentChannel](./API_RESOURCES.md#agentchannel) for extraction configuration.
   ```json
   {
     "messageId": "uuid",
     "channelType": "webhook",
...
**Per-replica observation list.** Each replica keeps a bounded list of in-window observations per registered channel path, each shaped `{ result: success | failure, reason, timestamp, lastError? }`. Entries older than the window are dropped on insertion or on read. A successful observation requires both: webhook auth validation passed **and** the message was dispatched to the target agent (`POST /v1/message` returned 2xx, or — in async mode — was queued for the retry pipeline). Failures past the auth step but before agent dispatch (e.g., agent not Ready, route resolution failed) are recorded as `failure` with the corresponding reason. Outbound callback attempts that fail the deny-range / allowlist re-check immediately before dial are additionally recorded as `failure` with `reason: CallbackInvalid` — this is the only callback-side failure that contributes to channel health, since it indicates a structural misconfiguration the operator must fix; transient callback delivery failures are recoverable via the retry pipeline and are not recorded. No etcd writes are performed per request.
...
2. Else any replica reports `failure` ⇒ `PlatformConnected = False` with the most recent failure's reason (`WebhookAuthFailed`, `AgentNotReady`, `DispatchFailed`, `CallbackInvalid`).
...
| Sync-mode wall-clock budget exceeded | Sync callers receive `504` with `error.type: sync_deadline_exceeded`, `retryable: true` when total wall-clock exceeds `gateway.syncDeliveryDeadline` (default 30s) mid-retry. Async mode is unaffected (no wall-clock budget). See [Request Flow step 6a](#user-gateway--request-flow). |
