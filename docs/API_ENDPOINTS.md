...
|---|---|---|
| 400 | no | `invalid_request` — `webhook.userId.fromBody` or `webhook.content.fromBody` is configured on the AgentChannel but the inbound body is not parseable as JSON. The body is structurally malformed; retrying without changing the body hits the same condition. See [User Gateway Error Responses](#user-gateway-error-responses) |
...
| 202 | — | Request accepted; agent response not yet available — caller is expected to poll again after a short delay |
