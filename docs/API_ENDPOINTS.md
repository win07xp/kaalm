...
| 504 | `wake_timeout` (hibernated agent did not reach Ready within `wakeTimeout`), `controller_unavailable` (activator endpoint unreachable; gateway sets `Retry-After: 5`), **or** `sync_deadline_exceeded` — total sync-mode wall-clock (delivery retry pipeline + agent processing) exceeded `gateway.syncDeliveryDeadline` (default 30s); `retryable: true` (the failure is timing-related, not structural — a fresh attempt within the deadline may succeed). Sync-only by construction; async mode is unaffected (callback / polling are receiver-driven and not bounded by a wall-clock budget) |
