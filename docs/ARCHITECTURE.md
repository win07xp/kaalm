...
- Per-Deployment `ServiceAccount`s, `ClusterRole`s, and `ClusterRoleBinding`s, plus companion namespaced `Role`s/`RoleBinding`s in `agentry-system` for the grants that must not be cluster-wide (Leases, Secrets, ConfigMaps — see [SECURITY.md § RBAC Model](./SECURITY.md#rbac-model))
...
1. [**Agent Reconciler**](./CONTROLLER_RECONCILERS.md#agentreconciler) — watches `Agent` resources. Provisions the [per-Agent child-resource set](#per-agent-and-per-task-child-resources) and drives the [persistent-agent state machine](./CONTROLLER_LIFECYCLE.md#agent-persistent-mode) (idle detection, hibernation, wake-on-demand).

2. [**AgentTask Reconciler**](./CONTROLLER_RECONCILERS.md#agenttaskreconciler) — watches `AgentTask` resources. Provisions the [per-AgentTask child-resource set](#per-agent-and-per-task-child-resources) to execute the task, monitors the [completion condition](./CONTROLLER_LIFECYCLE.md#agenttask) (`agentReported` or `exitCode`), and tears down resources on completion or timeout.
...
- **NetworkPolicy as the cross-tenant boundary.** Synthesized per-Pod NetworkPolicies bound Agentry-managed Pods' egress to the AgentClass-permitted set — gateway-mediated for LLM provider traffic (no direct provider egress) plus any AgentClass-allowed direct egress (`allowedCIDRs`, `allowedHosts` — e.g., MCP servers) (see [Per-Agent and Per-Task Child Resources](#per-agent-and-per-task-child-resources)). Gateway-only-tier Pods are not Agentry-managed and inherit no synthesized policy — see the caveat under [Adoption Tiers](#adoption-tiers).

- **Webhook path namespace-prefixing.** AgentChannel webhook paths must be namespace-prefixed (`/channels/{namespace}/...`), enforced at reconcile time (`Ready=False, reason=InvalidPath`) and re-checked by the gateway at path registration ([rule 15](./API_RESOURCES.md#cross-resource-validation) — the rule is not expressible in CRD CEL, which cannot read `metadata.namespace`), so cross-tenant path collisions are impossible at the routing layer. `AgentChannel.spec.agentRef` is name-only and binds to an Agent in the AgentChannel's own namespace; cross-namespace channel→Agent binding is not supported. See [API_RESOURCES.md § AgentChannel](./API_RESOURCES.md#agentchannel).
...
- **One Pod** containing the user's agent container, present only while the Agent is `Running`. The Pod runs under the [RuntimeClass](./SECURITY.md#runtimeclass) specified by its AgentClass, if one is set (e.g. gVisor or Kata); when unset it runs under the cluster's default container runtime.
- **One PVC** if the [Agent spec requests persistence](./API_RESOURCES.md#spec-2), mounted into the agent container at a configured path — provisioned by the controller, or a pre-existing claim referenced via `persistence.existingClaim` (no provisioning, no ownerRef; see [API_RESOURCES.md § Agent](./API_RESOURCES.md#agent)).
...
All of the above resources live in the same namespace as the Agent CR and carry an ownerRef back to it; full Agent deletion cascade-GCs them. There is no per-Agent configuration ConfigMap: non-sensitive config (gateway endpoint, ports) is delivered as env vars injected at Pod creation, and config changes are Pod-replacing spec drift by design — the same model stated for AgentTask below.

On `Hibernated`, only the Pod is deleted; the PVC, per-Agent `Certificate` (and its Secret), Service (with no endpoints), ServiceAccount, and NetworkPolicy are retained so wake-on-demand can recreate the Pod against unchanged identity and storage. See [CONTROLLER_LIFECYCLE.md § Hibernation mechanics](./CONTROLLER_LIFECYCLE.md#hibernation-mechanics).

AgentClass changes propagate to existing Agents along one of three paths, depending on whether the change constrains the derived Pod spec, excludes the Agent's stored spec, or only affects per-request routing.

**1. Pod-spec changes propagate via controller-side recreate-and-clamp.** When the platform team mutates an in-use AgentClass field that constrains the derived Pod spec without excluding the Agent's stored spec (`maxLimits` lowered, `runtime` changed, `security` tightened, `network.egress.allowedCIDRs` reduced, `image.allowedImages` narrowed but still admitting the Agent's `image`), the controller re-derives the [child-resource set](#per-agent-and-per-task-child-resources) for every Agent referencing the class — clamping `resources.limits` to the new `maxLimits`, applying the tighter `securityContext`, regenerating the per-Agent `NetworkPolicy` — then deletes and recreates the Pod, preserving PVC, Service, and Certificate. In-flight AgentTasks are not interrupted: they finish under their original class spec, and the new invariants apply only when the task next provisions a Pod (a backoff retry or a subsequent AgentTask CR). This makes AgentClass a live policy lever for Agents without disrupting one-shot task work. Bulk-impact and rollout guidance: [CONTROLLER_LIFECYCLE.md § AgentClass change handling](./CONTROLLER_LIFECYCLE.md#agentclass-change-handling).

**2. Class-vs-Agent-spec mismatches propagate as a `Degraded` transition.** Four kinds of change exclude an Agent's spec rather than just constrain its derived Pod: the Agent's `spec.image` is no longer in `image.allowedImages`, its `spec.providers` references a ModelProvider no longer in `allowedProviders` or whose own `allowedNamespaces` no longer includes the Agent's namespace, `spec.persistence.enabled: true` against a class with `persistence.enabled: false`, or `spec.lifecycle.hibernationEnabled: true` against a class with `lifecycle.hibernationAllowed: false`. The reconciler does not recreate the Pod. It transitions the Agent to `phase=Degraded` with a `reason` naming the specific mismatch (`ClassConstraintViolation` for image/provider, `PersistenceNotAllowed` for persistence, `HibernationNotAllowed` for hibernation), records the prior phase in `status.preDegradedPhase`, and resumes normal operation when the developer aligns the Agent or AgentClass spec. The same handling applies whether the mismatch is observed at initial provisioning or introduced by class drift on an already-running Agent. AgentTask, which has no `Degraded` phase, transitions to `phase=Failed` with the same `reason` instead — see [Cross-Resource Validation](./API_RESOURCES.md#cross-resource-validation) rules 2, 4, 5, 24, 26 and [CONTROLLER_LIFECYCLE.md § AgentClass change handling](./CONTROLLER_LIFECYCLE.md#agentclass-change-handling).

**3. Routing-concern fields propagate via the gateway's CRD and Secret watches with no Agent-side effect** — `spec.models` shrinkage, fallback-chain edits, credential rotations on `credentialsRef` (ModelProvider), and *additive* changes to `allowedNamespaces` or `allowedProviders` that don't exclude any currently-bound provider or namespace. These take effect on the next routed call without any Pod-level transition. Restrictive changes to `allowedNamespaces` or `allowedProviders` that exclude a currently-bound provider or namespace fall under bucket 2's `Degraded` handling, not this one — bucket membership depends on whether the change excludes a stored spec value, not on the field's nominal category. See [GATEWAY_LLM.md](./GATEWAY_LLM.md) for the per-request routing and credential mechanics.
...
- [Identifies the source namespace and workload (name and kind)](./GATEWAY_LLM.md#namespace-identification) via mTLS client-cert SAN (Agentry-managed Agent/AgentTask Pods), or namespace only via `TokenReview`-validated ServiceAccount bearer token (gateway-only tier), with a source-IP → Pod cross-check from the informer cache as defense in depth. On `POST /v1/task/complete` only, the gateway also performs a live API-server `List Pods` fallback when the informer cache misses, so the new-Pod startup race window collapses into the retryable `403 StalePodCompletion` path rather than leaking as a terminal `401` (see [API_ENDPOINTS.md § POST /v1/task/complete](./API_ENDPOINTS.md#post-v1taskcomplete-agenttask-only)); cost is one List per cross-check miss — bounded by the AgentTask Pods in the namespace, rare in practice, and the existing cluster-wide Pod read in the gateway's RBAC surface (see [RBAC](#control-plane) below) covers it without new permissions. The workload-identity half of the SAN underpins downstream workload-level checks (`spec.providers`, the per-task ConfigMap targeting, and the Agent-vs-AgentTask handler-level split below); gateway-only callers carry no workload identity, which is the structural reason their tenancy chain in [Multi-tenancy § Provider access gating](#multi-tenancy) reduces to `ModelProvider.allowedNamespaces` alone
...
- Delivers the message to the agent container via [`POST /v1/message`](./API_ENDPOINTS.md#post-v1message-agent-only--agent-implemented). Delivery is **bidirectional mTLS**: the gateway presents its per-Deployment cert (client; the same cert from [Control Plane](#control-plane), shared across replicas with Service DNS in the SAN) and verifies the agent's per-Agent cert against the Agentry CA; the agent verifies the gateway's client cert via the same CA, requiring a SAN match on the gateway Service DNS (specified in [RUNTIME_CONTRACT.md](./RUNTIME_CONTRACT.md), bullet 4). This is layered with the synthesized [NetworkPolicy gateway → agent ingress allow rule](#per-agent-and-per-task-child-resources). See [GATEWAY_USER.md § Request Flow step 6](./GATEWAY_USER.md#user-gateway--request-flow).
- Supports per-AgentChannel sync (default) and async response modes. In async mode the gateway returns `202 Accepted` with a `requestId` immediately and runs delivery and response handling in the background — [configuration](./API_RESOURCES.md#spec-4), [request flow](./GATEWAY_USER.md#user-gateway--request-flow), [response schemas](./API_ENDPOINTS.md#async-webhook-response-gateway-managed). Sync mode is bounded by `gateway.syncDeliveryDeadline` (default 30s) and is positioned for fast webhooks where the agent is `Running` and replies within seconds; async is the recommended mode when wake-on-demand or agent processing would routinely exceed that bound, since the deadline tighter than `wakeTimeout` (default 120s) and the agent-delivery retry budget makes `wake_timeout` / `delivery_failed` unreachable on the sync path under defaults — see [API_ENDPOINTS.md sync-mode response table reachability note](./API_ENDPOINTS.md#post-channelsnamespacechannel-path-external--webhook-caller)
...
See [API_ENDPOINTS.md § /v1/task/complete](./API_ENDPOINTS.md#post-v1taskcomplete-agenttask-only) and [§ /v1/agent/heartbeat](./API_ENDPOINTS.md#post-v1agentheartbeat-agent-only) for the handler-level caller-type checks. `/v1/task/complete` additionally enforces a Pod-identity check (calling Pod's UID must match `AgentTask.status.currentPodUID`, rejected with `403 StalePodCompletion`) and a terminal-phase check (`status.phase ∈ {Succeeded, Failed, TimedOut}` rejected with `403 TaskAlreadyCompleted`), both gated **after** the AgentTask SAN admission and the `agentReported` mode check — see [Per-Agent and Per-Task Child Resources](#per-agent-and-per-task-child-resources) for the field-stamping protocol.
...
- [**Async webhook responses**](./GATEWAY_USER.md#user-gateway--request-flow): the receiving replica creates an empty placeholder per-request ConfigMap in `agentry-system` at 202-acceptance time so the polling endpoint can return `202` for in-flight requests, then `Patch`es the same ConfigMap with the agent's response (or a delivery error) when the response will not be delivered via callback — either because no `callbackUrl` is configured, or because the bounded callback-retry schedule was exhausted. The placeholder and final ConfigMap share one bounded expiry (set at placeholder creation, not reset on patch) and the originating channel's namespace and name labels. The gateway rejects agent responses above a configured size limit with a `response_too_large` error returned to the caller (sync) or delivered to `callbackUrl` / the polling endpoint (async). Any replica answers a [polling-endpoint GET](./API_ENDPOINTS.md#async-webhook-response-gateway-managed) by reading this ConfigMap, so polling is replica-agnostic and no in-memory routing is required. The [AgentChannelReconciler](./CONTROLLER_RECONCILERS.md#agentchannelreconciler) prunes expired entries by label selector on reconcile passes (bounded by the [reconciler's requeue interval](./CONTROLLER_RECONCILERS.md#reconcile-interval-and-performance), so worst-case lingering is bounded by that interval past expiry); on AgentChannel delete, the [finalizer](./CONTROLLER_LIFECYCLE.md#finalizers) sweeps the channel's remaining response ConfigMaps in one shot — load-bearing because a cross-namespace ownerReference back to the AgentChannel is invalid: the GC would resolve the owner in the ConfigMap's namespace (`agentry-system`), treat it as missing, and delete the ConfigMap immediately (`OwnerRefInvalidNamespace`), so these ConfigMaps carry no ownerRef at all and the label linkage plus reconciler sweep is the only cleanup path.
