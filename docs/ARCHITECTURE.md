...
The controller's [RBAC surface](./SECURITY.md#operator-serviceaccount) covers cluster-scoped CRD watches, child-resource management, dynamic per-channel and per-task `Role`/`RoleBinding`s in user namespaces, and Pod read/list/watch in `agentry-system` for the activity-API fan-out.
...
The gateway's [RBAC surface](./SECURITY.md#gateway-serviceaccount-permissions) covers cluster-scoped `create` on `tokenreviews` for the gateway-only tier, the cluster-wide Pod watch (used for the source-IP → Pod cross-check called out under [LLM Gateway](#the-agentry-gateway) as defense in depth) and the cluster-wide AgentChannel watch for routing, per-task `Role`/`RoleBinding`s granting name-scoped `update`/`patch` on each `{taskName}-completion` ConfigMap, and per-AgentChannel `Role`/`RoleBinding`s granting name-scoped `get` on each AgentChannel's webhook auth Secret in the agent's namespace (both issued at reconcile time and cascade-deleted with their owners).

**Multi-replica state.** The gateway runs as multiple replicas. Each piece of per-namespace state uses one of three reconciliation strategies — cross-replica ConfigMap with a controller-side reducer (spend), local recomputation against a cluster-wide ceiling (rate limits), or per-replica in-memory state merged controller-side at read time (activity):
