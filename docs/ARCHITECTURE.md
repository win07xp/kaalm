...
Agentry ships as a Helm chart. **cert-manager, trust-manager, and an NP-enforcing CNI are required prerequisites** — see [SECURITY.md § Network Policy](./SECURITY.md#network-policy) and the NetworkPolicy bullet under [Per-Agent and Per-Task Resources](#per-agent-and-per-task-resources). The chart supports the two [Adoption Tiers](#adoption-tiers): the gateway-only tier requires only the gateway and provider Secrets in `agentry-system`, while the full lifecycle tier additionally enables the operator's Agent / AgentTask / AgentChannel reconcilers and the cert-manager / trust-manager prerequisites for per-Pod mTLS. The gateway-only tier's egress responsibility is stated under [Adoption Tiers](#adoption-tiers).
...
- **Provider access gating.** For full-lifecycle agents, an Agent can use a provider only when both `AgentClass.allowedProviders` and `ModelProvider.allowedNamespaces` admit it — both must pass. Gateway-only-tier callers have no Agent or AgentClass to consult, so they are gated by `ModelProvider.allowedNamespaces` alone. See [API_RESOURCES.md § AgentClass](./API_RESOURCES.md#agentclass).
...
- [Resolves the target ModelProvider](./GATEWAY_LLM.md#provider-routing) from the qualified `provider/model` name and the Agent's `spec.providers`, then [validates model and namespace access](./GATEWAY_LLM.md#request-format-detection) — see [Multi-tenancy § Provider access gating](#multi-tenancy)
