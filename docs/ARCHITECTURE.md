...
- Controller and Gateway Deployments — both with a PodDisruptionBudget (`minAvailable: 1`) and a rolling-update strategy; the Controller's PDB keeps the activator endpoint reachable across voluntary disruptions, since a brief activator gap surfaces as `controller_unavailable` 504s for any in-flight webhook to a hibernated Agent. Plus their ServiceAccounts, ClusterRoles, and ClusterRoleBindings
