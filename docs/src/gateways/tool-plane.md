...
Cluster-scoped because tool access is platform policy, exactly as model access is: the platform team registers the capability once, holds its credential in `kaalm-system`, and namespaces are admitted to it. The health probe speaks the protocol it governs, in whichever revision the server does (see [Protocol Revisions](#protocol-revisions)): it opens with `server/discover`, the request the 2026-07-28 revision requires every server to answer, and a success makes the rest of the probe stateless (`tools/list` carrying its version in `_meta`). Any answer that is not a recognized modern JSON-RPC error identifies a legacy server, per the revision's own compatibility rules, and the probe falls back to the `initialize` handshake followed by `tools/list`, exactly the pre-graduation probe. Either way `Healthy` surfaces the way ModelProvider's upstream probe does, and the probe records the negotiated revision in `status.mcpRevision`, so an operator can see which era each server speaks and when the fleet has moved. The name ToolProvider (over ToolServer) is deliberate: the symmetry with ModelProvider is the design, and the resource provides tools the way a ModelProvider provides models.
...
**Session ownership (legacy revisions).** Through the 2025 revisions, MCP sessions ride an `Mcp-Session-Id` header, and a broker that passes them through verbatim would let one workload resume another's session. The 2026-07-28 revision removes protocol-level sessions outright, so this control applies exactly as long as the header does; on modern traffic there is no session to own, and the ownership property it protected is carried by the per-request caller identity the broker authenticates on every call anyway. With replicated gateways, an in-memory binding table breaks on cross-replica routing, so the binding is stateless: the broker never reveals the upstream session id. What the agent receives is a wrapped id binding the upstream id to the caller's identity under an HMAC keyed by gateway-shared material, `wrapped = base64(upstreamID) || hmac(key, upstreamID || callerIdentity)`. On every subsequent request, any replica recomputes the HMAC from the presented id and the authenticated caller identity; a mismatch is rejected before anything is forwarded. No shared state, no session table, no cross-replica coordination, and a session is unresumable by anyone but its owner by construction.

### Protocol Revisions

MCP revision **2026-07-28** makes the protocol stateless: the `initialize` handshake and the `Mcp-Session-Id` header are gone, every request carries its version and capabilities in `_meta` (mirrored into an `MCP-Protocol-Version` header), and a new required `server/discover` RPC advertises what a server supports. Two of its additions are spec-level recognition of what this broker already is: `Mcp-Method` is required on every streamable-HTTP POST and `Mcp-Name` on `tools/call`, `resources/read`, and `prompts/get`, so an intermediary can route and police without parsing bodies; and list results carry `ttlMs` and `cacheScope`, so an intermediary knows what it may cache. A twelve-month deprecation window keeps 2025-era servers legitimate until roughly mid-2027, so Kaalm is dual-era for the length of the window: the probe negotiates per server as described above, and the broker enforces per request.

**The request's own era selects the posture.** The broker keys on the request's `MCP-Protocol-Version` header, which the revision requires and the spec tells intermediaries to check before trusting mirrored headers. On a modern request (2026-07-28 or later) the broker validates what the revision makes mandatory: `Mcp-Method` must be present and match the body's `method`, and `Mcp-Name` must be present and match `params.name` on `tools/call`, rejecting mismatches or absences with `400` and JSON-RPC error `-32020` (`HeaderMismatch`), as the spec requires of any body-processing server. The body stays the source of truth for policy, because the body is what the upstream executes; the headers are a required consistency proof, not a substitute. Validated headers, and any `Mcp-Param-*` headers a tool schema mirrors from call arguments, are forwarded upstream, where the server performs the same validation. On a legacy request (no version header, or an earlier version) none of these headers exist to check, and the broker behaves exactly as it did before the revision, session wrap included. The broker never translates between eras: a legacy caller against a modern-only server fails in the protocol's own vocabulary, which is the compatibility matrix's answer, not a gap in the broker.

**Caching truth for a filtered list.** The broker's `tools/list` relay narrows the catalog to the caller's grant, which turns a possibly-`public` upstream answer into a caller-specific one. On modern responses the broker therefore rewrites `cacheScope` to `private` before relaying, so no shared cache between the broker and the model ever serves one workload's tool surface to another. Legacy responses carry no such field and get none invented for them.
...
v0.6.0 adds the dual-era posture of [Protocol Revisions](#protocol-revisions): the `server/discover` probe with `initialize` fallback, `status.mcpRevision`, per-request era enforcement with the mirrored-header validation, the `cacheScope` rewrite, and the legacy scoping of session ownership. The 2025-era paths (handshake, session wrap) are removed together with the revisions that need them, no earlier than the upstream deprecation window closes, and their removal is a documented breaking change for tool servers, not for Kaalm's own API.
