...
Every brokered call emits a structured audit record: timestamp, caller identity (name, kind, and namespace for mTLS callers; namespace for the bearer tier), ToolProvider, tool name, JSON-RPC method, outcome, duration, and request and response sizes. Bodies are never logged: tool-call content is exactly the category the [log-redaction rule](../operations/observability.md#pii-safety) names as sensitive. The bodylog debug facility extends to the MCP routes under its existing gate, for operators who accept that trade during an investigation.

Metrics: `kaalm_tool_calls_total{provider, namespace, tool, status}` and a duration histogram per (provider, tool), added to the catalogs when v0.4.0 lands. The `tool` label's cardinality is bounded by the server's catalog; declared `spec.tools` catalogs are recommended for exactly this reason, per the [cardinality rules](../operations/observability.md#cardinality).
...
This chapter is design, merged in v0.3.0; every mechanism above lands with the v0.4.0 milestone, tracked in its umbrella issue. Delivery is nearly complete: the [ToolProvider resource](../resources/toolprovider.md), its reconciler, the grant fields on both workload kinds, reconcile-time enforcement of rules 35 to 38, and the broker itself (`/v1/mcp/*` with call-time enforcement at both protocol surfaces, credential injection, stateless session ownership, and rate limits) all exist, and `Agent.spec.mcpServers` is gone. What remains is the audit record and metric surface, and the on-cluster S18 proof. The scenario below and its coverage row carry the same annotation, so the acceptance surface stays honest about what is proven versus designed.

## Acceptance Scenario

[S18](../appendix/scenarios.md#s18-grant-an-agent-a-governed-tool) exercises the plane end to end: a platform engineer registers a ToolProvider whose credential lives in `kaalm-system`, a class allows it, an agent lists and calls tools through the gateway with no tool credential in its pod, a namespace outside the allowlist is denied, and a call to an ungranted tool is rejected with the distinct status. The e2e spec that will prove it on a cluster lands with v0.4.0 and is tracked in the [scenario coverage map](../appendix/scenario-coverage.md).
