...
Metering is **rate limits and audit, not budgets**. Tool calls carry no token-price dimension, and rule 33's own argument cuts against pretending: an unpriced call costs zero in a ledger, and a cap over zeros is a lie. v0.4.0 ships per-(namespace, ToolProvider) rate limits reusing the token-bucket machinery the LLM plane already divides across replicas. USD budgets for tools are future work whose schema slot is already named (`costPerCallUSD`, above, for provider-side tools; a per-call cost on the ToolProvider catalog is the analog when someone needs it), not a v0.4.0 promise.
...
| Tool server unreachable or refusing | `503 tool_unavailable`, retryable |
