...
The LLM Gateway listener serves TLS to protect LLM request and response payloads in transit within the cluster. Without TLS, prompts and completions traverse the cluster network in plaintext, which is unacceptable when agent containers run untrusted code on shared nodes. `$KAALM_GATEWAY_ENDPOINT` is an `https://` URL: TLS is not optional.
...
1. **mTLS-required** for Kaalm-managed Agent/AgentTask requests (Mode 1, see [Namespace Identification](workload-identity.md)).
...
The same cert is used as a client cert when the agent calls `$KAALM_GATEWAY_ENDPOINT` (see [Namespace Identification](workload-identity.md)). Only the first of those SANs is a shape the gateway's identity extractor recognizes; the two short forms match no recognized suffix and are ignored.
