...
![Activity diagram of the gateway's namespace-identification decision tree on :8443. If a client certificate is presented, the request takes Mode 1: the cert is verified against agentry-ca at the handshake (a failure there is rejected with no HTTP response at all), the DNS SAN list is scanned for a recognized suffix (.svc.cluster.local or .task.agentry.io, with short-form Service SANs ignored), and exactly one recognized SAN with exactly the expected label count of 5 is required for either shape, with anything else rejected as 403 invalid_cert; the namespace is then the SAN label at index 1. If no client certificate is presented, the request takes Mode 2, which begins with the uncached Pod-ownership precheck: the source IP is resolved to a Pod, and a Pod carrying an ownerRef to an Agent or AgentTask, or the Agentry-managed label set, is rejected 401 regardless of the token presented, which is what blocks the auth downgrade. Only then does the gateway consult its token cache and, on a miss, POST a TokenReview with audience agentry-gateway; an unreachable apiserver yields 503 internal_unavailable with Retry-After: 1, an unauthenticated token yields 401, and success yields the namespace from the middle segment of status.user.username, cached with a TTL derived from the token's own exp claim minus a 60s margin and capped at 5 minutes. Both modes then converge on the source-IP-to-Pod cross-check, where a namespace mismatch is rejected 401.](../../diagrams/workload-auth-modes.svg)
...
- `{name}.{namespace}.task.agentry.io`: issued by the AgentTaskReconciler. AgentTasks have no Service, so a non-Service shape is used to make the workload type explicit rather than implying a Service the task does not have. Exactly **5 labels** when split on `.`.
