...
![Activity diagram of the gateway's namespace-identification decision tree on :8443. If a client certificate is presented, the request takes Mode 1: the cert is verified against kaalm-ca at the handshake (a failure there is rejected with no HTTP response at all), the DNS SAN list is scanned for a recognized suffix (.svc.cluster.local or .task.kaalm.io, with short-form Service SANs ignored), and exactly one recognized SAN with exactly the expected label count of 5 is required for either shape, with anything else rejected as 403 invalid_cert; the namespace is then the SAN label at index 1. If no client certificate is presented, the request takes Mode 2, which begins with the uncached Pod-ownership precheck: the source IP is resolved to a Pod, and a Pod carrying an ownerRef to an Agent or AgentTask, or the Kaalm-managed label set, is rejected 401 regardless of the token presented, which is what blocks the auth downgrade. Only then does the gateway consult its token cache and, on a miss, POST a TokenReview with audience kaalm-gateway; an unreachable apiserver yields 503 internal_unavailable with Retry-After: 1, an unauthenticated token yields 401, and success yields the namespace from the middle segment of status.user.username, cached with a TTL derived from the token's own exp claim minus a 60s margin and capped at 5 minutes. Both modes then converge on the source-IP-to-Pod cross-check, where a namespace mismatch is rejected 401.](../../diagrams/workload-auth-modes.svg)

Reading the diagram: the shape to notice on the Mode 2 branch is what sits above the `TokenReview` call. The Pod-ownership precheck is first and is never cached, while the token result immediately below it is cached. That asymmetry is deliberate: Pod identity at a source IP can change, and a cached precheck would let a compromised Agent Pod ride a cache hit into the bearer path it is supposed to be excluded from.

Both modes ride on the gateway's TLS listener, so every caller must also verify the gateway's serving certificate against the Kaalm CA. Kaalm-managed Pods find the bundle at `$KAALM_CA_CERT` (`/var/run/kaalm/ca.crt`); gateway-only workloads mount the `kaalm-ca` ConfigMap manually (see [Caller-side setup](#caller-side-setup)). See the [Agent Runtime Contract](../../runtime/contract.md).
...
The identity extractor scans the certificate's DNS SAN **list** for entries ending in a recognized shape suffix (`.svc.cluster.local` / `.task.kaalm.io`). The per-Agent certificate also carries short-form Service SANs (`{name}.{namespace}.svc` and `{name}.{namespace}`); these match no recognized suffix and are ignored by the extractor. Exactly one SAN must match a recognized shape, and it must have exactly the expected label count: 5 for both shapes (`{name}.{namespace}` plus the three suffix labels of `.svc.cluster.local` or `.task.kaalm.io`). A recognized-suffix SAN with extra (or fewer) labels, or a certificate with zero or multiple recognized SANs, is rejected as `403 invalid_cert`.
...
Starter templates (see [Starter Templates](../../runtime/starter-templates.md)) demonstrate client-cert presentation and the cert-file watch-and-reload pattern. Custom images must configure their HTTP client to present the cert when calling `$KAALM_GATEWAY_ENDPOINT`. See the [Agent Runtime Contract](../../runtime/contract.md).
...
Provider routing for Kaalm-managed Pods runs the full chain: Agent/AgentClass `allowedProviders`, then ModelProvider `allowedNamespaces`/`models`. See [Provider Routing](provider-routing.md).
