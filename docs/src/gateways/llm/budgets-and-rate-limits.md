...
A hard cap cannot be enforced by after-the-fact counting alone: by the time a settled cost lands on the counters, the spend has happened. Instead of estimating request costs up front, which is not possible in general and least of all for streams, hard mode changes behavior only inside a **boundary region** just below each `block` threshold, where the remaining headroom is small enough that uncoordinated concurrency could cross the ceiling.
