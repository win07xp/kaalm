...
`400` is terminal here where the callback table leaves it unlisted, because on this path the platform has validated the body and will validate it the same way again; the two platform conditions that matter both arrive as `400` (Discord's `Unknown Interaction`, Meta's error `131047` for a reply outside the 24-hour window). The only exception is Meta's rate-limit code `130429`, which also arrives inside a `400` and is retried.
