...
Wake-on-demand is a hard dependency on the controller. While the activator endpoint is unreachable, a hibernated Agent cannot receive a message and the caller gets `controller_unavailable`; Agents that are already `Running` are unaffected. The chart's two-replica floor on the controller exists to keep this endpoint reachable through drains ([Deployment](../../operations/deployment.md#the-two-deployments)), and the gateway retries a failed call once on a fresh connection so that a call landing on a draining replica reaches the other one.

The gateway wakes an Agent when the Agent's cached `status.phase` is `Hibernated` or `Hibernating` at delivery time ([Request flow](overview.md#request-flow), step 7). A message that arrives while the Agent is `Hibernating`, with its Pod being deleted, requests a wake like any other: the controller keeps the wake annotation until the Pod is gone and the Agent settles `Hibernated`, then resumes it ([Manual wake](../../controller/hibernation-and-wake.md#manual-wake)). The gateway watches no Endpoints and infers nothing from connect failures: a connect failure during delivery is an ordinary failed attempt, retried on the delivery schedule.
...
![Flowchart of how a wake fails for a message to a Hibernated or Hibernating Agent, in the order the gateway finds out, as two rows. Wake: activator configured, reachable, and answering 202, else controller_unavailable (sync 504 with Retry-After 5); Agent Service reachable within wakeTimeout, else wake_timeout (sync 504). Deliver: sync deadline still open, else sync_deadline_exceeded (504, sync only); delivered within four attempts, else delivery_failed (sync 502); otherwise the reply.](../../diagrams/wake-failures.svg)

| Failure | Trigger | Sync mode | Async mode |
|---|---|---|---|
| `controller_unavailable` | No activator configured; a `4xx`, such as a SAN rejection or a `404` for a deleted Agent; or a connect or TLS failure, a ten-second timeout, or a `5xx` on both attempts. No wake is requested. | `504`, `Retry-After: 5`, `retryable: true` | error payload to `callbackUrl` or the polling endpoint |
