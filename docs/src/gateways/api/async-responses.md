...
`502 delivery_failed` becomes reachable in sync mode only when `syncDeliveryDeadline` is raised above the delivery budget. `wakeTimeout` defaults to 120s (`2m`), inherited from `AgentClass.spec.lifecycle.defaultWakeTimeout` and capped by `maxWakeTimeout`; see [AgentClass](../../resources/agentclass.md). For which channels belong on sync mode, and what a persistent `sync_deadline_exceeded` means, see [Reachability under default config](channel-webhook.md#reachability-under-default-config).
