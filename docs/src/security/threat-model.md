...
| Generated code attempts a container escape | The class RuntimeClass (gVisor, Kata) isolates the kernel; the restricted Pod Security Standard the controller applies to every workload Pod prevents privilege escalation ([Pod Security Standards](model.md#pod-security-standards), [RuntimeClass](model.md#runtimeclass)); a class that relaxes it gets `SecurityBaseline=False` and a `Warning`. | Mitigated |
