...
1. **Orientation** explains the problem Agentry solves, the vocabulary used everywhere else (the five custom resources, the two gateways, the adoption tiers, workload identity, lifecycle phases), the personas the design serves, and the system topology.
...
3. **Agent Runtime** covers the contract a container image must satisfy to run under Agentry, the starter templates that implement it, and the child resources the operator manages for each agent.
...
If you are new to the project, read part 1 in order. If you are implementing a component, read parts 1 and 2 first, then the part for your component. If you are evaluating Agentry as an operator or security reviewer, parts 1, 6, and 7 are written for you.
