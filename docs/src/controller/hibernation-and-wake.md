...
![Activity diagram of the controller's decision after an activity fan-out. If no gateway replica is reachable the result is UNREACHABLE: the current phase is preserved, a GatewayReachable=False condition is set, and the reconcile requeues with backoff. Otherwise each reachable replica's replicaStartedAt is compared against the Agent's status.phaseTransitionTime. A replica that started more recently is classified UNKNOWN, because it restarted while the Agent was Running and its empty store is not evidence of silence; an older replica is classified SILENCE, having covered the whole period without recording activity. If every replica is UNKNOWN, or if no replica has been up for at least idleTimeout, the controller defers and makes no transition. Only past both gates is missing data read as genuine inactivity, at which point timestamps are merged most-recent-per-agent across replicas, the activitySource filter is applied, and idle and hibernation transitions are evaluated.](../diagrams/activity-data-missing.svg)

Reading the diagram: the rule "absence of data is not evidence of inactivity" is a three-way distinction. **Unreachable** means the controller has no answer at all. **Unknown** means a replica answered but its answer carries no information, because it restarted mid-Running and lost its in-memory store. **Silence** is the only classification that is real evidence, and it requires a replica that was actually watching for the whole window. Grey marks the states with no usable evidence; both of them defer.
