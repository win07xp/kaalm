...
Steps 1 through 4 are what make this a handshake rather than a plain delete: the reconciler announces intent, waits for the gateway to acknowledge that it has stopped writing, and only then cleans up. The 30s timeout is the escape hatch that keeps a dead gateway from wedging channel deletion forever.
