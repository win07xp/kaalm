...
3. **Pre-Pod checks**, run only while no Pod exists and the phase is `Pending` or `Provisioning`, so a retry is validated against the class as it now stands. A class-versus-spec violation under rules 2, 4, 5, 24, and 35 to 38 settles the task as terminal `Failed` with the violation's reason (`ClassConstraintViolation`, `PersistenceNotAllowed`, or `ToolNotInCatalog`), since AgentTask has no `Degraded` phase. A missing `imagePullSecrets` entry, read under the task's pull-Secret Role as for an Agent, sets `Ready=False, reason=ImagePullSecretMissing` and requeues in 30 seconds, and an empty image sets `Ready=False, reason=InvalidReference`; neither is terminal.
