...
2. Write the `FQDNPolicySupported` condition on every pass. With `allowedHosts` empty it is `True, reason=NoHostsRequested`. With hosts listed, the cached result of the CNI probe decides: `True`, or `False, reason=FQDNPolicyUnsupported` with a `Warning` event of the same reason. Either way the class still becomes `Ready=True`. The AgentReconciler and AgentTaskReconciler turn the hosts into each workload's CiliumNetworkPolicy when the probe answers `True`, and ignore them otherwise ([FQDN egress policy](../runtime/child-resources.md#fqdn-egress-policy)).
...
For each policy whose `action: degrade`, the reconciler computes `avgCost(model) = (costPer1MInputTokens + costPer1MOutputTokens) / 2` for the target and compares it with every other model in `spec.models`. If the target is not strictly the cheapest, it sets the `DegradeTargetNotCheapest` condition to `True` with `reason=CheaperModelAvailable` and, on the transition to `True` only, emits a `Warning` event with `reason=DegradeTargetNotCheapest` naming the cheaper alternative. A steady misconfiguration therefore emits one event, not one per pass. When every target is the cheapest again, the condition goes `False` with `reason=DegradeTargetCheapest`.
...
8. **Children.** Converge the ServiceAccount, Service, PVC, NetworkPolicy, and CiliumNetworkPolicy, each read before any write. See [Child-resource convergence](#child-resource-convergence).
...
- **NetworkPolicy.** One per Agent, converged in place, combining: (a) egress to the gateway Service on 8443; (b) DNS egress on UDP and TCP 53 to the Pods `controller.networkPolicy.dnsSelector` selects, by default `k8s-app: kube-dns` in `kube-system` ([Deployment](../operations/deployment.md)); (c) ingress from the gateway on `KAALM_HEALTH_PORT`; (d) `AgentClass.spec.network.egress.allowedCIDRs` as `ipBlock` rules; (e) ingress from the namespace's other agent Pods on `KAALM_HEALTH_PORT`, only when `allowSameNamespaceIngress: true`. `allowedHosts` is never synthesized; see AgentClassReconciler step 2.
- **CiliumNetworkPolicy.** Named `{agentName}-fqdn`, written only when the class lists `allowedHosts` and the shared CNI probe answers `True`: DNS egress through Cilium's DNS proxy plus a `toFQDNs` rule per host ([FQDN egress policy](../runtime/child-resources.md#fqdn-egress-policy)). It is converged in place and deleted when the host list becomes empty. The controller reads it straight from the apiserver and does not watch it, because the kind exists only on Cilium clusters. On a cluster whose probe answers `False` the controller never reads or writes the kind.
...
5. **Children.** Converge the ServiceAccount, NetworkPolicy, the CiliumNetworkPolicy when the class lists `allowedHosts` and the CNI probe answers `True`, the PVC when persistence is enabled, and, for `agentReported` tasks only, the completion mailbox with its Role and RoleBinding. See [Completion mailbox and per-task Role](#completion-mailbox-and-per-task-role).
