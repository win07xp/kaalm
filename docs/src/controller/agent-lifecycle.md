...
| `Hibernated` | `Resuming` | The `kaalm.io/wake: "true"` annotation, set by the gateway [activator](../gateways/user/activation-and-activity.md#the-activator) on a channel message, or by hand. The annotation set while `Hibernating` is kept and honored once the Agent is `Hibernated`. In any other phase it is removed with a `WakeIgnored` warning, silently in `Resuming`. |
