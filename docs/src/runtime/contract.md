...
Since v0.3.0, the published [reference base images](base-images.md) are the canonical implementations of this contract and the primary on-ramp: they embed the contract runtime, and a developer supplies only a handler (mounted from a ConfigMap via [`Agent.spec.handler`](../resources/agent.md), or layered on with `FROM`). The templates consume the same single-sourced runtime code, so the contract behavior is identical across base images, templates, and anything built from either.
