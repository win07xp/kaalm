...
Starter templates are minimal, working implementations of [The runtime contract](contract.md), intended to be adapted when an agent outgrows the [reference base images](base-images.md). The base images are the primary on-ramp; the templates are the rung below full custom images, for developers who need to restructure the runtime itself rather than only supply a handler. They are **not** a framework. Developers own their copy.
...
A custom agent image has to satisfy every item in [The runtime contract](contract.md). The template handles the repetitive and error-prone parts, so developers can replace the agent logic without rebuilding the contract.

The ten template items below map onto the contract as follows:

| Template item | Runtime contract section |
|---|---|
| 1. HTTPS serving on `$KAALM_HEALTH_PORT` | [HTTPS health endpoints](contract.md#1-https-health-endpoints) |
| 2. mTLS client-cert presentation | [Gateway communication](contract.md#3-gateway-communication) |
| 3. CA trust bundle | [TLS requirements](contract.md#tls-requirements), server verification |
| 4. Cert-file watch and reload | [Certificate reload on rotation](contract.md#certificate-reload-on-rotation) |
| 5. `/v1/message` handler skeleton | [Message endpoint](contract.md#4-message-endpoint) and [Message deduplication](contract.md#7-message-deduplication) |
| 6. mTLS verification on `/v1/message` | [Client-certificate verification on /v1/message](contract.md#client-certificate-verification-on-v1message) |
| 7. Readiness + liveness endpoints | [HTTPS health endpoints](contract.md#1-https-health-endpoints) |
| 8. Graceful SIGTERM | [Graceful SIGTERM handling](contract.md#2-graceful-sigterm-handling) |
| 9. Heartbeat loop | [Activity signal (Agent only)](contract.md#5-activity-signal-agent-only) |
| 10. Task-completion helper | [Completion signal (AgentTask only)](contract.md#6-completion-signal-agenttask-only) |
...
2. **mTLS client-cert presentation**: pre-configures the outbound HTTP client that calls `$KAALM_GATEWAY_ENDPOINT` with the same cert pair as the client certificate, so LLM requests, heartbeats, and task completion calls satisfy mTLS identity without per-call TLS setup.
...
The kubelet rotates projected Secret and ConfigMap volumes by atomically renaming the `..data` symlink under the mount directory; the layout is drawn under [TLS material layout](contract.md#tls-material-layout). The leaf files (`tls.crt`, `tls.key`, `ca.crt`) are symlinks and are never written in place. An inotify watcher attached to a leaf path therefore never sees `IN_MODIFY` on rotation, silently misses every rotation, and keeps serving an expired certificate until the process restarts.

The templates instead watch the **mount directory** (`/var/run/kaalm/`, the parent of `$KAALM_TLS_CERT` / `$KAALM_TLS_KEY` / `$KAALM_CA_CERT`) for `IN_CREATE` and `IN_MOVED_TO` events on the `..data` entry: fsnotify in Go, `watchdog` in Python, both anchored to the parent directory.
...
examples/
  starter-go/          # the template rung: imports the runtime module
    ...
    README.md
  starter-python/      # the FROM rung: the base image provides the runtime
    Dockerfile         # FROM kaalm-agent-python + COPY handler.py + ENV
    handler.py         # handle_message(envelope), replace this
    README.md
```

Neither template carries contract code of its own. The Python template is a `FROM ghcr.io/win07xp/kaalm-agent-python` build (source in `images/agent-python/`), and the Go template imports the [`agentruntime` module](base-images.md#the-go-image) (source in `agentruntime/`); the ten items above are supplied by the base image and the module respectively.
...
The contract runtime is **single-sourced** in the source of the [reference base images](base-images.md), and the templates are thin consumers of it rather than copies.
...
- **The Go template** imports the published Go runtime module instead of vendoring the runtime code: its own code is `main.go` wiring a handler into the runtime. It exists as the worked example of restructuring the runtime while keeping the contract.
