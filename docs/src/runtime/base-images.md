...
- **The class must allow it (rule 30).** `spec.handler` may only be set when the referenced AgentClass has `spec.image.allowHandlerMounts: true`. The default is `false`: `allowedImages` (rule 2) is an image review boundary, and a mounted handler injects new code into an image that review already approved, so the capability is a deliberate per-class grant, not ambient. Violations follow the class-mismatch handling of rules 24, 26, and 29: `phase=Degraded, reason=HandlerMountNotAllowed`, recoverable when the specs align. See [Cross-Resource Validation](../resources/validation-and-defaulting.md#cross-resource-validation).
