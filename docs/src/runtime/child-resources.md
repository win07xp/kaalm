...
- **The PVC** is mounted into the agent container at `spec.persistence.mountPath`, default `/var/agent/memory` ([Agent spec](../resources/agent.md#spec)). The reference runtimes keep their state file there only under the default path ([Memory and dedup persistence](base-images.md#memory-and-dedup-persistence)).
