...
The **e2e** column is the acceptance surface: every scenario names the spec that proves it on a cluster, and every spec is green. S1 to S20 and S22 to S24 run in `test/e2e/` against the locally built chart and images. S21 runs in `test/upgrade/` (`make e2e-upgrade` and the `upgrade` CI workflow), which starts from the previous released chart rather than the local build. The LLM scenarios run against an in-cluster mock provider that speaks both the Anthropic and the OpenAI formats, and the channel scenarios against a mock of each platform's API.
...
| S24 Cross-format fallback (Anthropic and OpenAI) | `Cross-format fallback (S24)` (against the mock provider speaking both `/v1/messages` and `/v1/chat/completions`: an `anthropic` primary at the mock's `/fail` falls back to an `openai-compatible` backup with a model map and the Anthropic caller gets an Anthropic-shaped reply naming the mapped model, non-streaming and as a translated event stream; an `openai-compatible` primary falls back to an `anthropic` backup with `max_tokens` supplied from the catalog and the OpenAI caller gets a chat completion; spend lands on the backup's `status.budgetUsage`; a request with extended thinking cannot cross, ends in `502 provider_error`, and the `FallbackIneligible` event on the primary names the feature) | Unit: the `internal/llmtranslate` suite (one case per matrix row both ways, the untranslatable lists, max_tokens, streams), `TestIntegration_CrossFormatFallback_*` and `TestIntegration_LegacyCompletionsNeverCross` (the walk, the adapters, spend, envelopes); envtest `TestModelProvider_CrossFormatFallbackIsReady`, `TestModelProvider_ModelMapMustNameRealModels`, `TestModelProvider_CrossingWithoutMaxOutputTokensWarns` (rules 12 and 41); the CEL fixture `modelprovider-crossing.yaml` |
...
- **S9**'s `VolumeSnapshot` step is standard Kubernetes, not a Kaalm mechanism,
