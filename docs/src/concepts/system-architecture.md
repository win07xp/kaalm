...
The controller dials each gateway Pod IP directly rather than the Service, because [activity timestamps are in-memory per replica](../gateways/overview.md#multi-replica-state) and a Service-routed request would reach only one of them. Replica IPs are enumerated from the controller's gateway-Pod informer over `kaalm-system`. That informer also backs the channel-health fan-out, and it is the operational reason for the cluster-wide Pod watch in the [RBAC surface](#rbac-surface) below. Dialing a Pod IP against a Service-DNS-scoped SAN needs a specific TLS-handshake detail: see [Activity tracking API](../gateways/user/activation-and-activity.md#activity-tracking-api). The same fan-out pattern is reused by channel health, see [Channel health tracking](../gateways/user/platform-adapters.md#channel-health-tracking).
