...
The gateway and the console write structured JSON to stdout at `info` through `log/slog`. As shipped the controller writes controller-runtime's development encoding, console-formatted text at `debug`, to stderr, because the manager sets zap's development mode and the chart passes no logging flag. The chart has no log-level or log-format value; the controller accepts controller-runtime's `--zap-log-level` and `--zap-encoder` flags, but no template accepts extra arguments, so setting them needs a Deployment patch. The chart configures no log shipping; ship the streams with a cluster log pipeline such as Fluent Bit, Vector, or Loki.
