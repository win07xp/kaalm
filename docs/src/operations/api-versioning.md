...
The `kaalm.io` API group serves two versions of every custom resource: `v1beta1`, the storage version and the version Kaalm promises to keep compatible, and `v1alpha1`, deprecated since v0.6.0, still served, and converted to and from `v1beta1` by the controller. Before v0.6.0 there was one version and no conversion webhook, and a breaking change meant a rename and reinstall.
...
The warning text has the same form for each of the six kinds. It is the only user-visible effect of the deprecation until removal, and it is deliberate: the cluster itself says which version to move to, so nobody has to read a release note to find out.

Kaalm's own components (the controller, the gateway, and the console) speak `v1beta1` only, and the e2e suite does too except where the upgrade test applies `v1alpha1` on purpose. Once every stored object is at `v1beta1`, the steady state has no conversions at all, and the conversion path serves only clients that still send `v1alpha1`.
...
The graduation was the moment for field cleanups collected from real usage, and the audit of the six kinds found nothing that justified a breaking change at the moment the API starts promising not to break. What was reviewed and kept:
...
The conversion path is built and proven while the conversion is a plain copy, so that when `v1` does change a field, the only new work is the conversion rule for that field.
...
Both directions are total: every value a `v1alpha1` object can hold has a `v1beta1` form and the reverse. Correctness is defined by two round-trip identities, and round-trip fuzz tests assert both for all six kinds on every test run. Identity means wire-form identity: the webhook only ever sees JSON, so the tests compare the serialized forms (and the fuzzed input is normalized through its own JSON round trip first, because a distinction JSON cannot carry cannot exist in any apiserver flow). A field added to one version and not the other fails the fuzz suite immediately, which is what turns the identical-schema statement above from an intention into an enforced invariant:
...
Trust runs one way. The apiserver verifies the controller against the `caBundle` cainjector keeps in each CRD (annotation `cert-manager.io/inject-ca-from: kaalm-system/kaalm-controller-tls`); the controller verifies nothing about the caller, because a `ConversionReview` carries no authority. It converts the bytes it is handed and returns them.
...
The upgrade e2e that proves [S21](../appendix/scenarios.md#s21-upgrade-in-place-and-keep-every-agent) runs the two steps against the release the `PREV_CHART_VERSION` make variable names and asserts the behavior appropriate to that release. As shipped the variable pins v0.7.0, which is after the graduation, so the default run asserts that no window exists and nothing migrates; the window and a real storage migration are exercised only when the variable names a release before v0.6.0.
...
The controller therefore runs a **storage-version migrator** on the replica that wins leader election, once per leader term (it is a write burst, and one writer is enough). In a fixed order (AgentClass, ModelProvider, ToolProvider, Agent, AgentTask, AgentChannel), for each CRD whose `status.storedVersions` is anything other than `["v1beta1"]`, it lists every object at `v1beta1` in pages of 500 and issues a no-op write for each (an empty merge patch at the storage version). The apiserver re-encodes an object on any write whose stored bytes differ from the new encoding, and a version change is such a difference, so the no-op write is enough to move the object. It changes no field in spec or status and leaves `metadata.generation` alone; it does bump `resourceVersion`, so watches fire once per object and the reconcilers see one no-change event each, which is harmless. When every object of a kind is rewritten, the migrator patches that CRD's `status.storedVersions` to `["v1beta1"]`.

The migrator is idempotent and fast: a CRD already at `["v1beta1"]` is skipped without a log line, an interrupted run resumes on the next leader term and finds the already-rewritten objects need nothing, and a thousand objects take seconds. It logs one line per migrated kind plus a summary line with the migrated and already-current counts, and counts the objects it moved in `kaalm_storage_migrated_objects_total{kind}` ([Controller observability](../controller/operations.md#observability)). It lives in the controller rather than in a Job or the optional cluster storage-version-migrator because it needs no new image and the cluster-level migrator is absent from most distributions. Its RBAC is narrow: `get` on `customresourcedefinitions` and `patch` on `customresourcedefinitions/status`, both scoped by `resourceNames` to the six Kaalm CRDs ([RBAC and authentication](../security/rbac.md#operator-serviceaccount)). It reads each CRD by name and never lists them.

Two failure modes are designed in. If a CRD's storage version is not `v1beta1` (the chart's CRDs were not applied before the release was upgraded), the migrator refuses that kind, because trimming `storedVersions` then would claim a migration that did not happen. As shipped the first kind that fails, for that reason or any other, ends the pass: the remaining kinds in the order are not attempted, the error is logged once, and the whole pass retries with exponential backoff from one second, capped at five minutes, rather than failing the controller. A migration that cannot complete is a logged, retried condition; the reconcilers run regardless. A clean pass ends the migrator for that leader term, so a CRD that gains a second stored version later is migrated on the next leader change.
...
**The `v1alpha1` window.** `v1alpha1` is deprecated from v0.6.0. It stays served and convertible through v1.0.0 inclusive. It is removed no earlier than the first minor release after v1.0.0, and only after a release whose notes announce the removal, so there is always at least one release between the announcement and the removal. Until then the `deprecationWarning` is the only change a `v1alpha1` client sees. The CRD is the authority on what is served at any moment: `kubectl get crd agents.kaalm.io -o jsonpath='{.spec.versions[*].name}'`. The removing release has one precondition beyond the storage migration: children created before v0.6.0 (Pods, Services, PVCs, ServiceAccounts, NetworkPolicies, Certificates, and the channel Roles) carry `ownerReferences` that name `kaalm.io/v1alpha1`, which the garbage collector can resolve only while that version is served, and the controller's child reconciliation does not rewrite an existing child's ownerReferences. That release rewrites them before the version goes, and the removal issue tracks it.
