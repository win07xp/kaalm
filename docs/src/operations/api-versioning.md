...
1. **Apply the CRDs from the new chart.** `kubectl apply --server-side --force-conflicts -f crds/` adds `v1beta1` to each CRD, makes it the storage version, marks `v1alpha1` deprecated, and adds the conversion stanza. `--force-conflicts` is part of the command, not an escape hatch: Helm owns every CRD field from the install, and the first server-side apply must take that ownership over. cainjector fills the `caBundle` immediately, because the `kaalm-controller-tls` Certificate already exists.
