...
That is a decision, not an omission. The graduation was the moment for field-shape cleanups collected from real usage, and the audit of the six kinds found nothing that justified a breaking change at the moment the API starts promising not to break. What was reviewed and kept:
...
The value of the graduation is the promise and the conversion path that backs it, not a reshape. The conversion path is built and proven while the conversion is trivially correct, so that when `v1` does change a shape, the only new work is the conversion rule for that field.
