...
**`controller.networkPolicy.dnsSelector`.** The object takes the form `{ namespaceLabels: {...}, podLabels: {...} }` and supplies the `namespaceSelector` and `podSelector` for the DNS egress rule. The default matches kubeadm, EKS, GKE, AKS, k3s, and the upstream CoreDNS chart. `namespaceLabels` must select at least one label; an empty `podLabels` allows port 53 to every Pod in the selected namespaces. A cluster whose DNS runs elsewhere sets both to match its DNS Pods, or agent Pods cannot resolve names. See [Protecting agent containers from LLM provider access](../security/credentials.md#protecting-agent-containers-from-llm-provider-access).
