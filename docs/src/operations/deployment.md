...
| `gateway.providerFirstByteTimeout` | `120s` | Intended to bound each upstream LLM-provider attempt. As shipped the chart does not pass the value to the gateway, which uses its own `--upstream-timeout` default of 120s for the whole call, streams included. |
