...
**Rule 23: Image pull Secrets named by the class must exist where the workload runs.** Every `AgentClass.spec.image.imagePullSecrets[*].name` must exist as a Secret in the referencing workload's namespace. *Reconcile time, before the Pod is created; `Ready=False, reason=ImagePullSecretMissing`, message naming the namespace and Secret.* AgentClass is cluster-scoped but Secrets are namespaced, and the controller never copies Secrets across namespaces ([AgentClass design notes](agentclass.md#design-notes)).
