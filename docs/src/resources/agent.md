...
spec:
  ...
  # (sub-agents, coding agents with IDE integration, pure webhook handlers).
  providers:
  ...
  persistence:
    ...
    # (for example a PVC restored from a VolumeSnapshot of a finished AgentTask's
  ...
  # Service exposure. The Service is always ClusterIP.
  service:
    enabled: true
    port: 8080

  # Tool grants: gateway-brokered access to ToolProviders,
...
This is enforced by a root-scoped CRD CEL rule. Validation rules are not allowed under `metadata`, and `metadata.name` (together with `generateName`) is the only metadata field reachable from the object root, so the rule sits at the root of the Agent schema:
...
Agents that do not call LLM providers (sub-agents, coding agents with IDE integration, pure message handlers) omit it entirely. When present, it is a flat list of provider references. All providers are routed through `$KAALM_GATEWAY_ENDPOINT`. The agent uses a qualified model name format (`{providerRef}/{modelId}`, for example `anthropic-shared/claude-opus-4-6`) in API calls to identify both the provider and model. See [Provider routing and adapters](../gateways/llm/provider-routing.md) for the full routing chain.
...
- AgentTask does not support `existingClaim`; task scratch storage is always task-owned.
...
Starter templates handle both uses; see [Starter templates](../runtime/starter-templates.md). Custom images must configure their HTTP client to present the client cert for all calls to the gateway and must watch the cert/key files for rotation updates.
