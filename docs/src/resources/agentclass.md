...
metadata:
  name: standard
spec:
  # Runtime backend. "pod" is the only accepted value; the CRD enum rejects
  # anything else at apply time.
  runtime:
    backend: pod
    # Optional. When set, must name a RuntimeClass that exists on the cluster
    # (for example gvisor): Pod admission fails with "RuntimeClass not found"
    ...
    # objects at all, so pinning "runc" here would break scheduling.
  ...
  # Tool access: which ToolProviders may workloads of this class be
  # granted (rule 37)? Empty list = none, the same default as
  # allowedProviders.
  ...
  network:
    egress:
      ...
      # governs other direct egress (MCP tool traffic goes through the gateway's
      # tool plane instead). Enforced on any CNI that implements standard
  ...
  lifecycle:
    ...
    # Accepted by the schema but not applied as shipped: the gateway reads the
    # Agent's own wakeTimeout and uses 2m when it is unset (rule 9 is unenforced).
...
status:
  ...
  agentsInUse: 14       # count of Agents using this class
  tasksInUse: 2         # count of AgentTasks using this class
```

The `Ready` condition reports whether every reference the class makes (its `allowedProviders` and `allowedToolProviders` lists) resolves to an existing provider. `agentsInUse` and `tasksInUse` count the Agents and AgentTasks using this class, which tells the platform team what a change to the class will affect.

## Design notes

### An image allowlist is effectively mandatory

`allowedImages` is mandatory in practice for real clusters. An empty list means "any image", and nothing warns about it. Leave it empty only in throwaway environments.
...
`allowedProviders` is the access control mechanism for LLM providers at the class level, but it is not the only gate in either direction.
...
The same chain exists for tools, gate for gate: the workload's `spec.tools`, this class's `allowedToolProviders` (rule 37), and the target `ToolProvider.allowedNamespaces` (rule 36), with the per-tool narrowing and catalog check (rule 38) layered on top. See [Grants](../gateways/tool-plane.md#grants).

See [Provider access gating](../concepts/tenancy-and-tiers.md#provider-access-gating) for the enforced chain end to end, including which gate produces which error, and [Provider routing and adapters](../gateways/llm/provider-routing.md) for how the gateway resolves each layer.
...
The CRD schema enforces this with an enum on the `runtime.backend` field (`+kubebuilder:validation:Enum=pod`). Any other value is rejected at apply time rather than surfaced as a reconcile error, which gives the author immediate feedback. The field exists so that another backend is an additive enum change on the frozen v1beta1 schema rather than a reshape; isolation today comes from `runtime.runtimeClassName`, see [Runtime isolation](../concepts/system-architecture.md#runtime-isolation).
