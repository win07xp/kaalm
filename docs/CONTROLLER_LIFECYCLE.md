...
Only `Failed` transitions trigger backoff retries; `TimedOut` is terminal. A timeout indicates the workload's wall-clock budget was insufficient, not a transient failure — retrying with the same `timeoutSeconds` would re-time-out without progress. Tasks that genuinely need a longer budget should raise `spec.completion.timeoutSeconds` and re-apply the CR.
