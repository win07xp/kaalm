...
**Degrade-when-irreconcilable.** If the new invariants exclude the Agent's spec rather than just constrain its derived Pod spec — the workload's `spec.image` no longer matches `image.allowedImages`, its `spec.providers` references a ModelProvider no longer in `allowedProviders`, its `spec.persistence.enabled: true` while the class has `persistence.enabled: false`, or its `spec.lifecycle.hibernationEnabled: true` while the class has `lifecycle.hibernationAllowed: false` — the reconciler does not recreate the Pod. It transitions the Agent to `Degraded` with a `reason` naming the specific mismatch (`ClassConstraintViolation` for image/provider, `PersistenceNotAllowed` for persistence, `HibernationNotAllowed` for hibernation) and a message naming the offending field. The same `Degraded` handling applies whether the conflict is discovered during initial provisioning (developer applies an Agent that already violates the referenced class) or via class drift on an already-running Agent. The developer must update the Agent spec to comply; the controller resumes normal operation on the next reconcile after the Agent spec is reconciled with the class. The pre-Degraded phase is preserved in `status.preDegradedPhase` per the standard Degraded handling. The persistence cross-check applies symmetrically to AgentTask; since AgentTask has no `Degraded` phase, the task transitions to `Failed, reason=PersistenceNotAllowed` instead — matching the existing class-drift handling for AgentTask backoff retries.
