...
- If the agent is in any non-`Hibernated` phase, the annotation is removed immediately and a Warning event (`reason=WakeIgnored`) is emitted without changing phase. (This is the defense against the gateway's lazy hibernation detection misfiring on transient network failures — see [GATEWAY_USER.md § Activator](./GATEWAY_USER.md#activator).)
