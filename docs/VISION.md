...
- The **LLM Gateway**: a replicated proxy Deployment in `agentry-system` that mediates all agent-to-provider traffic. It provides spend visibility, soft budget guardrails, rate limiting, fallback routing, and credential isolation. Callers authenticate via one of two modes — mTLS client certificates for Agentry-managed Pods, or `TokenReview`-validated ServiceAccount bearer tokens for existing workloads in the gateway-only tier. Using it is optional per agent. The gateway-only tier's access control is governed by `ModelProvider.allowedNamespaces` alone — AgentClass `allowedProviders` does not apply, since those workloads have no Agent resource. Platform teams who want AgentClass-scoped provider policy must onboard workloads through the full Agent lifecycle tier. The default-deny NetworkPolicy that prevents agents from reaching provider IPs directly is synthesized only for Agentry-managed Pods; gateway-only-tier workloads route through the gateway voluntarily, and platform teams that want to enforce that routing must add their own egress policy on those namespaces.
