...
5. When creating a Pod, inject controller-managed environment variables:
   ...
   - `$AGENTRY_PROVIDER_ENDPOINT` (only when `spec.providers` is non-empty): identical in value to `$AGENTRY_GATEWAY_ENDPOINT`. Provided as a convenience alias for agents whose primary concern is LLM traffic. Custom images that predate `$AGENTRY_GATEWAY_ENDPOINT` can continue using this name.
