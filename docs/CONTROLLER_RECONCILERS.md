...
3. Ensure the `agentry-channel-{channelName}-creds` Role exists in the AgentChannel's namespace **before** validating Secret contents — the reconciler does not have blanket Secret-read RBAC in user namespaces, so the Role is what gives it (and the gateway) access to the specific Secret. The Role grants `get, list, watch` scoped via `resourceNames` to the single Secret referenced by the active auth type, and is bound by two RoleBindings against it: one to the gateway ServiceAccount (`agentry-system/agentry-gateway`) and one to the operator ServiceAccount (`agentry-system/agentry-controller`). The Secret and key locations depend on `spec.webhook.auth.type`: for `bearer`, the Secret is `spec.webhook.auth.secretRef.name` and the key is `spec.webhook.auth.secretRef.key`; for `hmac`, the Secret is `spec.webhook.auth.hmac.secretRef.name` and the key is `spec.webhook.auth.hmac.secretRef.key`. If the auth type or Secret reference has changed since the last reconcile (detectable by comparing the current Role's `resourceNames` against the new Secret name), the reconciler deletes the old Role before creating the new one — this prevents the gateway and operator from retaining read access to a Secret either of them no longer needs. The Role name is deterministic so that successive reconciles are idempotent. After the Role exists, the reconciler reads the referenced Secret via this scoped path and validates: (a) the Secret exists — if missing, set `Ready=False, reason=CredentialsMissing` with a message naming the Secret; (b) the referenced key is present in the Secret's `data` — if absent, set the same `Ready=False, reason=CredentialsMissing` with a message naming both the Secret and the missing key. The reason code is shared deliberately so consumers parsing channel conditions see one stable code for "channel auth Secret unusable" rather than having to branch on missing-Secret versus missing-key. Both Role and RoleBindings are owned by the AgentChannel via `ownerRef` and cascade-delete on AgentChannel deletion.
