...
4. Converge the remaining child resources in the task's namespace: `Pod`, `PVC`, `ServiceAccount`, `NetworkPolicy`. All are owner-referenced to the AgentTask for cascade GC. (For `agentReported`-mode tasks, the `{taskName}-completion` ConfigMap and its scoped Role/RoleBinding were already created in step 3 alongside the Certificate; they are not (re-)created lazily by the gateway. `exitCode`-mode tasks have no equivalent.)
...
7. On `Completing`: artifact values are read from the pre-existing `{taskName}-completion` ConfigMap that was created in step 3. When the agent calls `POST /v1/task/complete`, the gateway updates this ConfigMap (via `update`/`patch` against the resource-name-scoped Role) with the completion payload. The reconciler watches for changes to this ConfigMap, reads artifact values, and populates `status.artifactValues`. No exec into the container is required, and the completion data survives Pod crashes or eviction.
