...
6. Converge the remaining child resources to match the desired phase: `Pod`, `PVC`, `Service`, `ConfigMap`, `ServiceAccount`, `NetworkPolicy`. All are owner-referenced to the Agent for cascade GC.
   - **Service**: ClusterIP, named `{agentName}` in the Agent's namespace, with `port: spec.service.port` (default 8080) and `targetPort: $AGENTRY_HEALTH_PORT` (default 8080). The two are decoupled deliberately so a developer can override the Service-facing port without changing the in-Pod listen port — the agent only ever binds `$AGENTRY_HEALTH_PORT`. See the [Agent CRD design notes](./API_RESOURCES.md#agent).
