...
3. **Loaded**: the gateway watches `AgentChannel` resources directly. When it sees a new or updated AgentChannel, it reads the referenced Secret from the agent's namespace using its scoped RBAC and holds the credential in-process for the channel adapter. The operator ServiceAccount also has a parallel scoped read path on the same Secret via a dynamic per-channel Role (see [Operator ServiceAccount](#operator-serviceaccount) above), used solely by the AgentChannelReconciler to validate that the configured `data` key exists; the operator does not retain credential material in memory.
