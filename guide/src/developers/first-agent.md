...
Start from a published reference base image: it implements the whole
runtime contract (a message endpoint, TLS with the cluster CA,
...
The `Phase` column walks from `Pending` through `Provisioning` (Pod, PVC,
Service, ServiceAccount, Certificate, and NetworkPolicy being created; the
Pod starts only after its certificate is issued) to `Running`. `Ready: True`
means the whole set is up. On a small cluster the walk takes about fifteen
seconds:
...
Your agent received everything it needs as environment and mounts: the gateway endpoint (`KAALM_GATEWAY_ENDPOINT`), its client
