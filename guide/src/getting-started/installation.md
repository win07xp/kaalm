...
Three things must exist in the cluster before Agentry; the chart installs none
of them:

1. **cert-manager**, running with `--enable-certificate-owner-ref=true`. Agentry
...
3. **A NetworkPolicy-enforcing CNI** (Calico, Cilium, or similar). Agentry
...
make e2e-deploy    # helm install into agentry-system
```
...
```bash
docker build -t <registry>/agentry-controller:dev --build-arg BINARY=manager .
docker build -t <registry>/agentry-gateway:dev --build-arg BINARY=gateway .
docker push <registry>/agentry-controller:dev
docker push <registry>/agentry-gateway:dev
```

Then install the chart from the checkout, pointing it at your images:

```bash
helm upgrade --install agentry charts/agentry \
  -n agentry-system --create-namespace \
  --set controller.image.repository=<registry>/agentry-controller \
  --set controller.image.tag=dev \
  --set gateway.image.repository=<registry>/agentry-gateway \
