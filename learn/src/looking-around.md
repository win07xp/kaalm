...
Four verbs do most of the work:

| Verb | What you are asking |
|---|---|
| `kubectl get` | What exists? |
| `kubectl describe` | Tell me everything about this one thing. |
| `kubectl apply` | Here is a description; create it. |
| `kubectl delete` | Remove this. |

Three more arrive later, each explained where it appears: `create` for an
object built from a file on your laptop, `patch` for a one-line edit, and
`port-forward` to reach something inside the cluster.
...
run, which model providers they may use, whether they are allowed to use
...
`apply` it. Those files are called **manifests**, and they are YAML. The
skeleton of every one:
