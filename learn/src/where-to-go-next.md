...
Not Kubernetes, particularly. You learned a handful of `kubectl` verbs, that spec is
...
controller reconciles, how the gateway routes and bills, how every workload
gets its certificate, and the twenty-four acceptance scenarios the whole
system is tested against. Start with its System architecture chapter.
...
starter templates and fully custom images.

Whatever the packaging, the image keeps handling the parts that are hard to
get right:

- TLS, with certificate rotation
- verifying the gateway
- deduplicating redelivered messages
- persisting state so hibernation does not lose the thread

The design book's The runtime contract chapter is the authority on what an
agent must do.
...
- **Certificates.** They were issued, rotated, and verified for you
  throughout, and the book never mentioned it beyond installing cert-manager.
- **A production install.** One laptop node, no ingress, no monitoring, timers
  set to seconds instead of half an hour. The guide's Observing the platform
  part is where the fleet gets a screen (the optional console), Grafana
